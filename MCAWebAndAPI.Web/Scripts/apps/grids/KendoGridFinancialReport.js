...
EPMO.Grids.displayFinancialReportGrid = function (divId, strList, strQuarter, strProject, strMain) {
    var tempList;
    ...
    function getCurrentPeriod(strQtr, strCheck) {
        ...
        if (EPMO.Utils.isEmpty(strQtr) || strQtr == "Q01" || strQtr == "Q02") {
    ...
    tempList = EPMO.Utils.isEmpty(strList) == true ? "QDR%20Commitment" : strList;
    _url = str.substring(0, str.length - intLength) + "/_api/web/lists/getbytitle('" + tempList + "')/Items?$top=10000&$select=*,WBSID/Activity," +
